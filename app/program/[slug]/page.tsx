...
function getInstructionStyle(instruction: string) {
  ...
  if (value.includes("watch all (skip episode") || value.includes("watch episodes")) {
...
export default async function StepDetailPage({
  ...
  const stepIndex = steps.findIndex((item) => item.slug === slug);
        ...
        <div className="mt-12 flex items-center justify-between gap-4">
          ...
          <div>
            {nextStep ? (
              ...
              <span className="text-sm text-gray-600">End of Naruto Part 1</span>
            )}
          </div>
        </div>
      </div>
